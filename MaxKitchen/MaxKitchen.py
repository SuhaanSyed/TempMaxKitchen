...
def detect(img_dir):
    ...
    connection = httplib.HTTPSConnection('northcentralus.api.cognitive.microsoft.com')# "northcentralus" will change depending on ENDPOINT
    connection.request("POST", (ENDPOINT+'face/v1.0/detect?%s') % params, body, headers) # this is the specific endpoint
...
def getName(person_Id):
    ...
    connection = httplib.HTTPSConnection('northcentralus.api.cognitive.microsoft.com')
    connection.request("GET", (ENDPOINT+"/face/v1.0/persongroups/"+group_id+"/persons/"+person_Id+"?%s") % params, "{body}", headers)
    response = connection.getresponse()
    data = json.loads(response.read())
    name = data['name']
    connection.close()
...
while True:
    ...
    if motion == notDetected: # does nothing if motion is not present
        ...
        iter(picDirectory, False) # False is used here because this is a regular face check
        ...
        if len(faceIdList) > 0: # if tehre are faces then the faces are sent to identifyFaces
            result = identifyFaces(faceIdList) #confidence value along with faceId
    
            if result[0][1] > .65: #if confidence is greater than 65% then getName is called
                name = getName(result[0][0])
                ...
                if motionStillPresent.count(1)/len(motionStillPresent) >=.6:  #declares overtime for motion present for more than 60%
                    checkIter += 1
                    ...
                    for check in range(5):
                        ...
                        camera.capture(dir)
                        sleep(2)
                    iter(tempCheckDirectory, True) # True is used to indicate overtime-facial recognition
                    if len(checkOvertime) > 0: #if checkOvertime list is not empty
                        if name == getName(identifyFaces(checkOvertime)[0][0]): #if name matches to initial name
                            msg = name + " crossed their time limit in the kitchen. They spent about " + totalTime((datetime.now()-now).seconds) + " in the " +location+". Please ask them to get out of there!" 
